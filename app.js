...
function currentDisplay(i) {
  for (j = (i - 1) * 10; j < i * 10; j++) {
    prevlink.setAttribute("href", `javascript:prev(${i-1});`);
    nextlink.setAttribute("href", `javascript:next(${i+1});`);
...
//function for the button previous and then recursing it
function prev(i){
    if(i!==0)
    prevlink.setAttribute("href", `javascript:currentDisplay(${i}); prev(${i-1});`);
}
prevlink.innerHTML = "❮";
...
for (i = 1; i < 11; i++) {
  ...
  link.setAttribute("href", `javascript:currentDisplay(${i}); javascript:prev(${i-1}); javascript:next(${i+1});`);
  link.innerHTML = i;
  li.append(link);
  ul.append(li);
}

//function for the button next and then recursing it
var nextli = myfunc("li", "page-item", "buttonnext");
var nextlink = myfunc("a", "page-link", "nextlink");
function next(i){
    if(i!==11)
    nextlink.setAttribute("href", `javascript:currentDisplay(${i}); next(${i+1});`);
}
nextlink.innerHTML = "❯";
